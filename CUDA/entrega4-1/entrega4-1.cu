...
/*
Programa en Cuda que realiza la convolución de matrices en GPU mediante un 
bloque y memoria global
*/
                            ...
                            int (*matrizR_d)[dimA])
{
    ...
    if (arriba >= 0)
    {
        ...
        valor1 = matrizA_d[arriba][threadIdx.x];
    ...
    if (izquierda >= 0)
    {
        ...
        valor1 = matrizA_d[threadIdx.y][izquierda];
    ...
    valor1 = matrizA_d[threadIdx.y][threadIdx.x];
    ...
    if (derecha < dimA)
    {
        ...
        valor1 = matrizA_d[threadIdx.y][derecha];
    ...
    if (abajo < dimA)
    {
        ...
        valor1 = matrizA_d[abajo][threadIdx.x];
...
int main(int argc, char **argv)
{
    ...
    // Ejecutar kernel de convolución en el dispositivo, un bloques con 32*32 hilos
    dim3 bloquesA(1, 1);
    dim3 hilosA(dimA, dimA);
    convolucion<<<bloquesA, hilosA>>>(matrizA_d, matrizBR_d, matrizR_d);
